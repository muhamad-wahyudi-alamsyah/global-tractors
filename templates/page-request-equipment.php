...
<body class="gti-body">
                                        ...
                                        <td class="col-actions">
                                            <div class="gti-ue-action-menu">
                                                ...
                                                <div class="gti-ue-action-dropdown">
                                                    ...
                                                    <button type="button" class="gti-ue-action-item delete" onclick="openDeleteModal(<?php echo esc_attr($req->id); ?>, '<?php echo esc_js($req->request_id); ?>', '<?php echo esc_js($req->equipment); ?>')">
        ...
        <div class="gti-ue-delete-modal">
            ...
            <div class="gti-ue-delete-body">
                ...
                <div class="gti-ue-delete-warning">
                    ...
                    <span>Request will be permanently removed from the system. This data cannot be recovered.</span>
                </div>
            </div>
    ...
    // ====== Delete Request — modal + toast ======
    var _deleteReqId = null;

    function openDeleteModal(requestId, requestCode, equipment) {
        _deleteReqId = requestId;
        document.getElementById('delete-eq-name').textContent = equipment || '—';
        document.getElementById('delete-eq-code').textContent = requestCode || '—';
    ...
    function handleDeleteConfirm() {
        if (!_deleteReqId) return;
        var confirmBtn = document.getElementById('gti-delete-confirm-btn');
        confirmBtn.disabled = true;
        confirmBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Deleting...';

        var deletedId = _deleteReqId;
        var formData = new FormData();
        formData.append('action', 'gti_delete_request');
        ...
        .then(function(res) {
            if (res.success) {
                closeDeleteModal();
                showUeToast(res.data?.message || 'Request deleted', 'success');
                var row = document.querySelector('tr[data-req-id="' + deletedId + '"]');
                if (row) {
                ...
                confirmBtn.disabled = false;
                confirmBtn.innerHTML = '<i class="fas fa-trash"></i> Delete';
                showUeToast(res.data?.message || 'Failed to delete request', 'error');
            }
        })
        .catch(function() {
            confirmBtn.disabled = false;
