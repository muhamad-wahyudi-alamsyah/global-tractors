...
function gti_fluentform_to_request($insertId, $formData, $form) {
    ...
    // request_id (REQ-XXXX) is generated by gti_insert_with_reference() at the
    // insert below: the previous read-then-write took the sequence from the
    // newest row rather than the highest number, so a legacy REQ-2026-001 row
    // sent it straight back to a taken sequence.
    ...
    gti_log('Mapped request', $data);

    $result = gti_insert_with_reference($table, $data, 'request_id', 'REQ-');

    if ($result) {
        $request_id = $result['ref'];
